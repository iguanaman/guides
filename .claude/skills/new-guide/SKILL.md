...
## 3. Research (find sources, then distill in parallel batches)
...
- Dispatch `guide-research-distiller` in parallel batches of 6 — 6 Agent calls in one message, wait for the batch, then send the next 6. Pass {topic}, `{topic-slug}`, the row's `slug` as `{source-slug}` (verbatim — it's the output filename), and the row's url. Distiller reads its one source and writes `{topic-slug}/.research/{source-slug}.md` before ending its turn.
- Batch rather than one all-at-once fan-out: a batch that dies mid-flight costs 6 re-runs, not the whole source list, and the diff step below catches the losses.
